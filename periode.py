...
t = np.arange(0, periode_new, 1) #temps entre 0 et la période divisé par 200 avec un pas de 1, cela correspond à t(0), t(1), t(2) jusqu'à t(période/200)
...
def graph_pendule():
    ...
    #la variable hémisphère sert à donner ou non l'opposé de la force de Corriolis selon si on se trouve sur l'hémisphère nord ou sud
    ...
    # on trace la courbe
    angle_cercle=np.linspace(0, 2*np.pi, 180)   #création d'un cercle de rayon z0 pour que les oscillations du pendule apparraissent dedans, " un peu comme au Panthéon, en 1851"
    rayon=z0_
    ...
    plt.axis("equal")   #pour que le graph soit proportionnel et pour ne pas afficher un cercle applati, égalise le pas de chaque axe
    ...
    with writer.saving(fig, "pendule.mp4", 300): #on choisis la figure à animer, le nom du fichier à sauvegarder et la résolution en dpi
        for i in range(periode_new):
            ...
            red_circle.set_data(x0, y0) #set.data attribue les valeurs chaques secondes
